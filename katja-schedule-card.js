...
class KatjaScheduleCard extends HTMLElement {
  ...
  _renderCalendarGrid(days, grouped, opts = {}) {
    ...
    // Collect distinct multi-day events (dt_end > start). They render
    // as continuous bars spanning day columns — the way the Starred D
    // Flow view does — instead of a repeated per-day chip with a
    // redundant "all day" tag. Take whichever copy is first seen
    // (start or continuation: a continuation carries _spanStart so we
    // can still reconstruct the span when the start day is off-view).
    // Drives + (unless revealed) flagged rows are excluded.
    const multiDay = [];
    const seenMd = new Set();
    for (const dayEvents of Object.values(grouped)) {
      for (const ev of dayEvents) {
        if (this._isDrive(ev.summary || "")) continue;
        if (!this._showFlagged && this._isFlagged(ev)) continue;
        const end = (ev._dtEnd || "").slice(0, 10);
        if (!end) continue;
        const start = ev._isContinuation
          ? (ev._spanStart || "").slice(0, 10)
          : (ev.start?.date || ev.start?.dateTime || "").slice(0, 10);
        if (!start || end <= start) continue;
        const key = ev._eventId || `${start}|${ev.summary || ""}`;
  ...
  _renderCalWeek(week, grouped, multiDay) {
    ...
    for (const md of multiDay) {
      ...
      bars.push({
        ev: md.ev,
        colStart: Math.max(0, this._daysBetween(weekMon, md.start)),
        colEnd: Math.min(6, this._daysBetween(weekMon, md.end)),
        continuesLeft: md.start < weekMon,
        continuesRight: md.end > weekSun,
      });
    }
    // Greedy track-pack: longest bars first; each takes the lowest
    // track whose occupied column ranges don't overlap it.
    bars.sort((a, b) => (b.colEnd - b.colStart) - (a.colEnd - a.colStart));
    const tracks = [];
    for (const b of bars) {
      let t = 0;
      for (; t < tracks.length; t++) {
        if (tracks[t].every(([s, e]) => b.colEnd < s || b.colStart > e)) break;
      }
      (tracks[t] = tracks[t] || []).push([b.colStart, b.colEnd]);
      b.track = t;
    }
  ...
  _getStyles() {
    ...
    return `
      ...
      /* Multi-day bar strip — same 7-col / 3px-gap geometry as the
         ...
         covers the gaps between them, so it reads as one continuous
      ...
      .cal-mdbar.continues-left { border-top-left-radius: 0;
        border-bottom-left-radius: 0; border-left: none; padding-left: 9px; }
      .cal-mdbar.continues-right { border-top-right-radius: 0;
